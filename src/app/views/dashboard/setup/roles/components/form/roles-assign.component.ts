      ...
      <form [formGroup]="rolesForm">
        <div class="form-group row required">
          <div class="input-group input-group-sm col-sm-3">
            <label class="col-form-label"><b>Lista de Modulos:</b></label>
          </div>
          <div class="col-sm-9 input-group input-group-sm input-group-rounded">
            <select class="form-control form-select form-control-sm" formControlName="Parent_id">
              <option value="" disabled="true"> Seleccione Módulo</option>
              <option *ngFor="let mod of modules" [value]="mod.id">{{mod.nombre}}</option>
            </select>
          </div>
<!--          <app-form-validate-errors [group]="rolesForm" [controlName]="'Parent_id'"></app-form-validate-errors>-->
      ...
      <div class="row">
        <div class="col-md-12">
          <div class="list-group">
            <a class="list-group-item" *ngFor="let m of menus; let i = index">
    ...
    <div class="modal-footer">
      <button type="button" class="btn {{ abcForms.btnCancel.class }} btn-sm" (click)="cancelForm()">
        <span class="{{ abcForms.btnCancel.icon }} lamb-icon"></span> {{ abcForms.btnCancel.label }} / Terminar
      </button>
